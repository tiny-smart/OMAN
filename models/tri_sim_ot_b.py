...
class GML(nn.Module):
    ...
    def forward(self, x_pre, x_cur, dist):
        ...
        # loss += torch.relu(torch.bmm(x3,x4.transpose(1,2)) - self.margin).sum() # / (len(x3) * len(x4))     # 上式是不是写错了？而且分母没写？
        loss_dict={
    ...
    def similarity_cost(self, x1, x2, gamma=10):
        '''
        :param x1: [B N D]
        :param x2: [B M D]
        :return: [B N M]
        ...
        dist = self.dist
        if dist.shape[1] == sim.shape[1]:
            sim = dist * sim
        else:
            sim = dist.transpose(1,2) * sim
        return 1 - sim
...
if __name__ == "__main__":
    ...
    pos_sim = 1 - similarity_cost(x1, x2).detach().cpu().numpy()[0]
    pos_match = np.argmax(pos_sim, axis=1)
    pos_match_acc = pos_match == np.arange(pos_match.shape[0])  # 需要一个匹配的先验（ID）？
    all_match_acc = len([i > pos_match.shape[0] for i in all_match[1][:pos_match.shape[0]]])
    all_match_acc = all_match_acc / pos_match_acc.shape[0]
